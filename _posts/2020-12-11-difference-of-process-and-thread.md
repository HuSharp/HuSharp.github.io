---
layout: post
title:  " 进程管理（五）进程类别 & 线程与进程的区别 "
date:   2020-12-11 19:53:19 +0800
categories:  Linux
tags: Linux 基础知识
author: Hu#
typora-root-url: ..
...
![image-20201207201108438](/assets/blog_image/2020-12-11-difference-of-process-and-thread/image-20201207201108438.png)
...
虽然我们在区分Linux进程类别, 但是我还是想说Linux下只有一种类型的进程，那就是 task_struct，当然我也想说 linux 其实也没有线程的概念, 只是将那些与其他进程共享资源的进程称之为线程。
...
**参看：[进程与线程的一个简单解释](http://www.ruanyifeng.com/blog/2013/04/processes_and_threads.html)** 

大神举得例子很有意思。 

- 任一时刻，CPU总是运行一个进程，其他进程处于非运行状态。

进程好似车间，线程好似车间工人；任务是有很多工人协同完成；车间内的空间设施对于工人都是共享的；有些房间比如厕所一次只能容纳一人，一个线程使用某些共享内存时,需加锁（互斥锁）。

[多进程和线程的区别 线程继承的部分](https://blog.csdn.net/yusiguyuan/article/details/39432377)

## 1、首先要明确进程和线程的含义： 

1、进程（Process）是具有一定独立功能的程序关于某个数据集合上的一次运行活动，是系统进行资源分配和调度的一个独立单位。与程序相比，程序只是一组指令的有序集合，它本身没有任何运行的含义，只是一个静态实体。进程是程序在某个数据集上的执行，是一个动态实体。它因创建而产生，因调度而运行，因等待资源或事件而被处于等待状态，因完成任务而被撤消，反映了一个程序在一定的数据集上运行的全部动态过程。

每个正在系统上运行的程序都是一个进程。每个进程包含一到多个线程。进程也可能是整个程序或者是部分程序的动态执行。

2、线程是一组指令的集合，或者是程序的特殊段，它可以在程序里独立执行。**也可以把它理解为代码运行的上下文。**所以线程基本上是轻量级的进程，它负责在单个程序里执行多任务。通常由操作系统负责多个线程的调度和执行。
...
[python中的协程](https://www.liaoxuefeng.com/wiki/897692888725344/923057403198272)

[谈谈协程](https://zhuanlan.zhihu.com/p/147608872)

协程这个概念的提出，使得程序逻辑更为清晰，执行更加可控。
协程实质上是一种在用户空间实现的协作式多线程架构。它不能让OS知道自己的存在，无法利用多核CPU/CPU的多线程支持；但这恰恰是它的优点。注意，我在这里的措辞是“协程不能让OS知道自己的存在”。

没错，每次执行中，协程之间的具体执行顺序可能千变万化；但**协程执行权切换**却**只会发生在用户明确放弃执行权之后**——比如你明确执行了yield语句时。



协程是一种抛弃了在CPU上并行执行能力的、协作式多任务的执行框架。
...
只要你记得在合适时机主动交出控制权，不要调用系统提供的、可能阻塞的API（而是使用协程库提供的非阻塞版、或者使用协程库推荐写法），你甚至可以让磁盘、CPU、网络等不同设备并行运行——这本就是操作系统给你提供一个虚拟的、可并行界面的背后原理。
...
![image-20201208101631236](/assets/blog_image/2020-12-11-difference-of-process-and-thread/image-20201208101631236.png)
