...
比较设计。不同的映射模式最大的区别就在于比较设计上。全相联因为是随机选取的 *cache* 行进行写入，因此没有行地址（索引），因此直接进行 8 个 *cache* 槽的**并发比较**，得到命中的*cache*行*Li*，并给出*Miss*和*Hit*信号。比较方法见图所示。
...
- 一个是当存在cache槽为空，空行的选择；
- 一个是当cache槽满时，淘汰行的选择

1. 在空行的选择中，通过优先比较器来实现，优先比较器通过比较，**输出的是索引较大的非 0 行**，因此先对所有行的标志位取反，取反后，若为1，则表示该行为空；若为0，则表示该行已经存在数据。若所有行都存在数据则 *FULL* 行满信号有效。
...
![image-20201030233822345](/assets/blog_image/2020-10-19-hust-cpu-study_3/image-20201030233822345.png)
...
151 全相联应用场合
 块映射灵活 ，一对多映射
 cache cache全部装满后才会出现块冲突 全部装满后才会出现块冲突 全部装满后才会出现块冲突 全部装满后才会出现块冲突
 块冲突的概率低 ，cache cache 利用率高
 淘汰算法复杂

